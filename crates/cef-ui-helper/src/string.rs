use crate::{CEFLIB, RefCountedPtr, Wrappable, Wrapped, ref_counted_ptr};
use cef_ui_sys::{
...
impl CefString {
    ...
    pub fn from_userfree_ptr_unchecked(ptr: cef_string_userfree_t) -> Self {
        ...
        unsafe {
            let worked =
                (lib.cef_string_utf16_set)((*ptr).str_, (*ptr).length, cef.as_mut_ptr(), 1);
    ...
    fn utf8_to_utf16(s: &str) -> cef_string_t {
        ...
        match unsafe {
            (lib.cef_string_utf8_to_utf16)(s.as_ptr() as *const c_char, s.len(), &mut ret)
        } {
